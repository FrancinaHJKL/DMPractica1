...
function agregar(req,res){
    ...
    .then(info =>{
        return res.status(200).send({mensaje:"la informacion se guardo con exito", info})
    })
    .catch(e =>{
...
function eliminarBebida(req,res) {
    ...
    .then(info=>{
        return res.status(200).json({message:"producto eliminado", info}).
        catch(e=>{return res.status(404).json({message:"error al eliminar producto", error: e})
        })
    })
}
  ...
  if (!Object.keys(nuevosDatos).length)
    return res.status(400).send({ message: "el cuerpo de la solicitud esta vacio" });

  palapaModel.findOneAndUpdate(filtro, nuevosDatos, { new: true })
    .then(bebida => bebida
      ? res.status(200).send({ message: "producto actualizado", bebida })
